...
bool create_directory(const char* path) {
    ...
    if (strcmp(path, "/") == 0 || strcmp(path, "") == 0) {
        printk("create_directory: Path is root or empty, no need to create\n");
        return true; // Root directory or empty path already exists or is invalid
    }

    char* path_copy = strdup(path);
    if (!path_copy) {
        printk("create_directory: Failed to allocate memory for path copy\n");
        return false;
    }
    ...
    if (len == 0) {
        printk("create_directory: Path is empty after trimming slashes\n");
        free(path_copy);
        return false;
    }
    ...
    if (!last_slash) { // Directory in root
        printk("create_directory: Creating directory in root\n");
        ...
        *last_slash = '\0';
        ...
        if (strlen(name) == 0) {
            printk("create_directory: Directory name is empty after splitting path\n");
            free(path_copy);
            return false;
        }
        ...
        while (remaining && *remaining) {
            ...
            if (ret != UFAT_OK) {
                printk("create_directory: Failed to navigate: %s\n", remaining);
                free(path_copy);
                return false;
            }
            if (!(ent.attributes & UFAT_ATTR_DIRECTORY)) {
                printk("create_directory: Path component is not a directory: %s\n", remaining);
                free(path_copy);
                return false;
            }
            ret = ufat_open_subdir(&uf, &parent_dir, &ent);
            if (ret != UFAT_OK) {
                printk("create_directory: Failed to open subdirectory: %s\n", remaining);
                free(path_copy);
                return false;
            }
    ...
    if (ret == UFAT_OK) {
        printk("create_directory: Successfully created directory: %s\n", name);
    } else if (ret == UFAT_ERR_FILE_EXISTS) {
        printk("create_directory: Directory already exists: %s\n", name);
    } else {
        printk("create_directory: Failed to create directory: %s, error: %d\n", name, ret);
    }
...
bool create_file_impl(const char* path) {
    printk("create_file_impl: Attempting to create file at path: %s\n", path);

    char* path_copy = strdup(path);
    if (!path_copy) {
        printk("create_file_impl: Failed to allocate memory for path copy\n");
        return false;
    }
    ...
    if (len == 0) {
        printk("create_file_impl: Path is empty after trimming slashes\n");
        free(path_copy);
        return false;
    }
    ...
    if (!last_slash) {
        printk("create_file_impl: Creating file in root directory\n");
        ufat_open_root(&uf, &parent_dir);
        name = path_copy;
    } else {
        *last_slash = '\0';
        name = last_slash + 1;
        if (strlen(name) == 0) {
            printk("create_file_impl: Filename is empty after splitting path\n");
            free(path_copy);
            return false;
        }
        ...
        if (strlen(parent_path) == 0) {
            printk("create_file_impl: Parent directory is root\n");
            ufat_open_root(&uf, &parent_dir);
        } else {
            struct ufat_directory current_dir;
            ufat_open_root(&uf, &current_dir);
            ...
            do {
                ...
                if (ret != UFAT_OK) {
                    printk("create_file_impl: Failed to find path component: %s\n", remaining);
                    free(path_copy);
                    return false;
                }

                if (!(ent.attributes & UFAT_ATTR_DIRECTORY)) {
                    printk("create_file_impl: Path component is not a directory: %s\n", remaining);
                    free(path_copy);
                    return false;
                }
                ...
                if (ret != UFAT_OK) {
                    printk("create_file_impl: Failed to open subdirectory: %s\n", remaining);
                    free(path_copy);
                    return false;
                }
    ...
    if (ret == UFAT_OK) {
        printk("create_file_impl: Successfully created file: %s\n", name);
    } else {
        printk("create_file_impl: Failed to create file: %s, error: %d\n", name, ret);
    }
...
bool create_file(const char* path) {
    printk("create_file: Attempting to create file at path: %s\n", path);

    // Ensure the parent directory exists
    char* path_copy = strdup(path);
    if (!path_copy) {
        printk("create_file: Failed to allocate memory for path copy\n");
        return false;
    }
    ...
    if (last_slash) {
        *last_slash = '\0'; // Split into parent directory and filename
        ...
        if (strlen(parent_path) == 0) {
            ...
            printk("create_file: Parent directory is root, no need to create\n");
        } else {
            if (!create_directory(parent_path)) {
                printk("create_file: Failed to ensure parent directory exists: %s\n", parent_path);
                free(path_copy);
                return false;
            }
        }
...
bool write_file(const char* path, const void* data, uint32_t size) {
    printk("write_file: Attempting to write to file: %s\n", path);

    // Validate input
    if (!path || !data || size == 0) {
        printk("write_file: Invalid input parameters\n");
        return false;
    }
    ...
    int ret = ufat_dir_find_path(&dir, path, &ent, &remaining);
    if (ret != UFAT_OK) {
        printk("write_file: Failed to find file: %s\n", path);
        return false;
    }

    // Verify it's a file
    if (ent.attributes & UFAT_ATTR_DIRECTORY) {
        printk("write_file: Path is a directory, not a file: %s\n", path);
        return false;
    }
    ...
    ret = ufat_open_file(&uf, &file, &ent);
    if (ret != UFAT_OK) {
        printk("write_file: Failed to open file: %s\n", path);
        return false;
    }
    ...
    if (ret != UFAT_OK) {
        printk("write_file: Failed to truncate file: %s\n", path);
        return false;
    }
    ...
    if (bytes_written == size) {
        printk("write_file: Successfully wrote %d bytes to file: %s\n", bytes_written, path);
        return true;
    } else {
        printk("write_file: Failed to write data to file: %s (expected %d bytes, wrote %d)\n", path, size, bytes_written);
        return false;
    }
}
...
bool delete_file(const char* path) {
    printk("delete_file: Attempting to delete file: %s\n", path);
    ...
    // Verify it's a file
    if (ent.attributes & UFAT_ATTR_DIRECTORY) {
        printk("delete_file: Path is a directory, not a file: %s\n", path);
        return false;
    }
    ...
    if (ret == UFAT_OK) {
        printk("delete_file: Successfully deleted file: %s\n", path);
    } else {
        printk("delete_file: Failed to delete file: %s\n", path);
    }
